...
async def useradd(client, message: Message, _):
    if MONGO_DB_URI is None:
        return await message.reply_text(
            "**Bot'un gizlilik sorunları nedeniyle, Miss'in Veritabanını kullanırken sudo kullanıcılarını yönetemezsiniz.\n\n Bu özelliği kullanmak için lütfen değişkenlerinizde MONGO_DB_URI'nizi doldurun**"
        )
...
@app.on_message(filters.command(SUDOUSERS_COMMAND) & ~BANNED_USERS & filters.user(OWNER_ID))
@language
...
@app.on_message(
    filters.command(ID_COMMAND) & SUDOERS
)        
